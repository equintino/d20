...
<?php if($act === "add"): ?>
    <fieldset class="fieldset">
        <form id="form-breed" method="POST" action="breed/save" enctype="multipart/form-data" >
            <legend>Inserir Raça</legend>
            <label>Tipo:</label>
            <input class="form-control" type="text" name="name" />
            <label>Classe:</label>
            <input class="form-control" type="text" name="class" />
            <label>Gênero:</label>
            <input class="form-control" type="radio" name="sex" value="M"/>
            <label>Maculino:</label>
            <input class="form-control" type="radio" name="sex" value="F"/>
            ...
            <div style="float: right">
                ...
                <button type="submit" class="button btn-danger" >Salvar</button>
            </div>
        </form>
    </fieldset>
<?php elseif($act === "list"): ?>
    <section class="wrapper">
        <div id="imageBreed">
        <?php foreach($list as $breed): ?>
            <img id="<?= $breed["id"] ?>" src="image/id/<?= $breed["image_id"] ?>" alt="" />
        <?php endforeach; ?>
        </div>
    </section>
<?php else: ?>
    <!-- <fieldset class="fieldset" style="background: #e0dede;"> -->
        ...
        <div id="edit">
            <section>
                ...
                <textarea rows="5" cols="48" type="text" name="description" style="text-transform: none" ><?= $breed->description ?></textarea>
            </section>
